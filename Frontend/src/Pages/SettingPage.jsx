...
import LoadingSpinner from '../components/common/LoadingSpinner'; // Assuming this is available
...
const SettingPage = () => {
    ...
    const { user: loggedInUser } = useSelector((state) => state.auth); // Get logged-in user from Redux
    ...
    const handleThemeSelect = (t) => {
        if (t.isPremium && !loggedInUser?.isPremium) {
            toast.info(
            ...
            <div className={`min-h-screen ${theme.background} ${theme.text} flex flex-col lg:flex-row`}>
                ...
                <aside className={`w-full lg:w-64 xl:w-72 py-6 px-4 lg:px-6 lg:py-8 border-r ${theme.border}/50 ${theme.cardBg}/50 backdrop-blur-sm transition-all duration-300 ease-in-out`}>
                    <div className="flex flex-col justify-between h-full">
                        {/* Profile Summary (Optional: Add a brief summary if needed) */}
                        <div className="flex flex-col items-center mb-6">
                            <div className="relative group">
                                <img
                                    src={loggedInUser?.avatar || '/path/to/default/avatar.png'} // Use loggedInUser's avatar
                                    alt="User Avatar"
                                    className={`w-20 h-20 rounded-full border-4 ${theme.border}/30 shadow-lg object-cover`}
                                />
                            </div>
                            <h3 className={`mt-3 font-semibold text-lg ${theme.text}`}>{loggedInUser?.firstName} {loggedInUser?.lastName}</h3>
                            <p className={`text-sm ${theme.cardText}/80`}>{loggedInUser?.role || 'User'}</p>
                        </div>
                        ...
                        <nav className="flex flex-col space-y-3">
                            <button
                                ...
                                className={`flex items-center px-4 py-3 rounded-lg transition-all duration-300 text-left ${activeSettingTab === 'profile'
                                    ...
                                    : `${theme.text} hover:${theme.primary}/20 hover:${theme.highlight}`
                                    }`}
                            >
                            ...
                            <button
                                ...
                                className={`flex items-center px-4 py-3 rounded-lg transition-all duration-300 text-left ${activeSettingTab === 'theme'
                                    ? `${theme.primary} ${theme.buttonText} shadow-lg`
                                    : `${theme.text} hover:${theme.primary}/20 hover:${theme.highlightTertiary}`
                                    }`}
                            >
                            ...
                            <button
                                ...
                                className={`flex items-center px-4 py-3 rounded-lg transition-all duration-300 text-left ${activeSettingTab === 'account'
                                    ? `${theme.primary} ${theme.buttonText} shadow-lg`
                                    : `${theme.text} hover:${theme.primary}/20 hover:${theme.highlightSecondary}`
                                    }`}
                            >
                        ...
                        {/* Logout Button (Optional: can be in header or here) */}
                        <div className="mt-auto pt-8">
                            <button
                                ...
                                className={`${theme.secondary} ${theme.buttonText} px-6 py-3 rounded-lg font-medium hover:${theme.secondaryHover} transition-all duration-300 w-full`}
                            >
                                Logout
                            </button>
                        </div>
                ...
                <main className={`flex-1 p-6 lg:p-8 ${theme.background}`}>
                        ...
                        <section className={`${theme.cardBg} rounded-xl border ${theme.border} p-6 shadow-lg`}>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <p className={`font-medium ${theme.text}`}>{loggedInUser?.firstName}</p>
                                </div>
                                <div>
                                    <label className={`block text-sm font-medium ${theme.cardText} mb-1`}>Last Name</label>
                                    <p className={`font-medium ${theme.text}`}>{loggedInUser?.lastName || '-'}</p>
                                </div>
                                <div>
                                    <label className={`block text-sm font-medium ${theme.cardText} mb-1`}>Email</label>
                                    <p className={`font-medium ${theme.text}`}>{loggedInUser?.emailId}</p>
                                </div>
                                <div>
                                    <label className={`block text-sm font-medium ${theme.cardText} mb-1`}>Role</label>
                                    <p className={`font-medium ${theme.text}`}>{loggedInUser?.role}</p>
                                </div>
                                {/* Add other profile details if needed */}
                            </div>
                            <Link to="/profile/edit" className={`mt-6 inline-block px-6 py-3 rounded-lg font-medium text-sm ${theme.primary} ${theme.buttonText} hover:${theme.primaryHover} transition-all duration-300 shadow-md`}>
                                Edit Profile
                            </Link>
                        </section>
                    )}
                                ...
                                {THEMES.map((t) => (
                                    <div
                                        ...
                                        className={`rounded-xl border-2 transition-all duration-300 relative overflow-hidden
                                            ...
                                            ${t.cardBg}
                                            ${t.isPremium && !loggedInUser?.isPremium ? 'opacity-60 cursor-not-allowed' : 'cursor-pointer hover:scale-105'}`}
                                        onClick={() => handleThemeSelect(t)}
                                    >
                                        {/* Premium Overlay - Visible ONLY if theme is premium AND user is NOT premium */}
                                        {t.isPremium && !loggedInUser?.isPremium && (
                    ...
                    {activeSettingTab === 'account' && (
                        <section className={`${theme.cardBg} rounded-xl border ${theme.border} p-6 shadow-lg`}>
                            ...
                            <form onSubmit={handlePasswordSubmit} className="space-y-4 mb-8 pb-6 border-b border-neutral-700">
                                <h3 className="text-lg font-medium">Change Password</h3>
                                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                    <div>
                                        <label className={`block text-sm font-medium ${theme.cardText} mb-2`}>Current Password</label>
                                        <input
                                            type="password"
                                            value={passwordData.currentPassword}
                                            onChange={(e) => setPasswordData({...passwordData, currentPassword: e.target.value})}
                                            className={`w-full ${theme.cardBg}/80 border ${theme.border} rounded-lg px-4 py-3 ${theme.text} focus:outline-none focus:ring-2 focus:ring-blue-500 transition-colors`}
                                            placeholder="Enter current password"
                                            required
                                        />
                                    </div>
                                    <div>
                                        <label className={`block text-sm font-medium ${theme.cardText} mb-2`}>New Password</label>
                                        <input
                                            type="password"
                                    ...
                                    <div>
                                        <label className={`block text-sm font-medium ${theme.cardText} mb-2`}>Confirm New Password</label>
                                        <input
                                            type="password"
                                ...
                                <button
                                    ...
                                    className={`${theme.primary} ${theme.buttonText} px-6 py-3 rounded-lg font-medium hover:opacity-90 transition-all duration-300 disabled:opacity-50`}
                                >
                            ...
                            <section className={`bg-red-50 dark:bg-red-900/20 p-6 rounded-xl border border-red-200 dark:border-red-800 shadow-lg`}>
                                ...
                                <button
                                    ...
                                    className="bg-red-600 hover:bg-red-700 text-white font-medium py-2.5 px-6 rounded-lg transition-colors duration-300"
                                >
                                    Delete My Account
                                </button>
                            </section>
                    ...
                    <div className={`${theme.cardBg} rounded-xl border border-red-300 dark:border-red-700 w-full max-w-md`}>
                        <div className="p-6">
                            ...
                            <div className="space-y-4">
                                <div>
                                    <label className={`block text-sm font-medium ${theme.cardText} mb-1`}>Password</label>
                                    <input
                                        type="password"
                                ...
                                <div>
                                    <label className={`block text-sm font-medium ${theme.cardText} mb-1`}>
                            ...
                            <div className="flex justify-end space-x-3 mt-6">
                                <button
                                    ...
                                    className={`px-4 py-2 rounded-lg border ${theme.border} ${theme.cardText} hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors`}
                                    disabled={isDeleting}
                                >
                                    Cancel
                                </button>
