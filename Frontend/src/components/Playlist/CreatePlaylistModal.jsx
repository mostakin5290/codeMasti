// components/modals/CreatePlaylistModal.jsx
import React, { useState } from 'react';
import { FaPlus, FaTimes } from 'react-icons/fa';

const CreatePlaylistModal = ({ isOpen, onClose, onCreate, appTheme }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        setError(null);
        try {
            await onCreate({ name, description }); // Call the prop function
            setName(''); // Clear form on success
            setDescription('');
            onClose(); // Close modal
        } catch (err) {
            setError(err.message); // Error message from the ProblemPage handler
        } finally {
            setLoading(false);
        }
    };
    ...
    const getAccentColorBase = () => {
        ...
        return match ? match[1] : 'blue'; // Default to 'blue'
    };
